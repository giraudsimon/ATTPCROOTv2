...
void SetERtable(){//fit of the GEANT4 E vs R obtained with the simulation with the function model given by LISE++
...
void runana_digi_d2He()
{
	...
	std::string digiFileName = "../digi_test.root";
	...
	TFile* outfile;
	TString  outFileNameHead = "hist_digi_test.root";//attpcsim_d2He_test20k
	...
	Double_t Ekin_proj = 100.0 * 14.008596359;
		...
		if(fATRansac==NULL) std::cout<<" Null pointer fATRansac "<<"\n";
			...
			if(i%2!=0){
				...
				for(Int_t iHit=0; iHit<nHits; iHit++){
					...
					//if(i==13){ //i==13 "good 2p track exemple"
				...
				if(lines.size()>0){
					...
					if(lines.at(0).MaxR.first>=242. || lines.at(0).MaxR.second>=242. || lines.at(0).MaxR.first<=25 || lines.at(0).MaxR.second<=25 || lines.at(0).LastPoint.first.Z()>973. || lines.at(0).LastPoint.second.Z()>973. || lines.at(0).Range.first>1055. || lines.at(0).Range.second>1055.) continue; //if do no stop in chamber or in the beam hole
					...
					range_p1 = lines.at(0).Range.first;
					range_p2 = lines.at(0).Range.second;
